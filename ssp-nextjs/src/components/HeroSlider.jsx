  ...
  return (
    ...
    >
      {heroSlides.map((slide, index) => (
        ...
        >
          ...
          <div className="relative h-full max-w-7xl mx-auto px-4 md:px-8 flex items-center">
            <div
              className={`max-w-xl transition-all duration-700 ${index === currentSlide ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
            ...
            >
              ...
              <p className="text-lg md:text-xl mb-2 font-medium" style={{ color: '#F4C430' }}>{slide.subtitle}</p>
              <p className="text-base md:text-lg mb-8 text-gray-300 leading-relaxed">{slide.description}</p>
              {slide.link.startsWith('#') ? (
                <button
                  onClick={() => { const el = document.getElementById(slide.link.slice(1)); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }}
                ...
                >
                  {slide.cta} <ChevronRight size={16} className="inline ml-2 transition-transform duration-300 group-hover:translate-x-1" />
                </button>
              ) : (
      ...
      <button onClick={() => { prevSlide(); resetTimer(); }}
        ...
        aria-label="Previous slide" data-testid="hero-prev-btn">
        <ChevronLeft size={18} />
      </button>
      <button onClick={() => { nextSlide(); resetTimer(); }}
        ...
        style={{ backgroundColor: 'rgba(128, 0, 32, 0.7)', backdropFilter: 'blur(4px)' }}
        aria-label="Next slide" data-testid="hero-next-btn">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-20">
        {heroSlides.map((_, index) => (
          <button
            ...
            className={`h-1 rounded-full transition-all duration-500 ${index === currentSlide ? 'w-8' : 'w-3'}`}
